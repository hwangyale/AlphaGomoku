...
public:
	...
	void develop(int actions[], int values[], int count, std::unordered_map<U64, bool> &cache_hashing_table);
...
void FastNode::develop(int actions[], int values[], int count, std::unordered_map<U64, bool> &cache_hashing_table)
{
...
int fast_evaluate(FastBoard &board, int depth, int max_depth, int actions[],
				   std::unordered_map<U64, bool> &cache_hashing_table, int player)
{
	...
	static int tmp_actions[10];

	actions[0] = 0;

	if (player_vct.find(zobrisKey) != player_vct.end())
	{
		actions[++actions[0]] = player_vct[zobrisKey];
		return 1;
	}
	...
	{
		board.get_fast_actions(true, OPEN_FOUR, actions, actions[0]);
		return board.player == player ? 1 : 0;
	}

	if (board.count_actions(true, FOUR) > 0)
	{
		board.get_fast_actions(true, FOUR, actions, actions[0]);
		return board.player == player ? 1 : 0;
	}

	if (board.count_actions(false, OPEN_FOUR) > 0)
	{
		board.get_fast_actions(false, OPEN_FOUR, actions, actions[0]);
		return board.player != player ? 1 : 0;
	}

	if (board.count_actions(false, FOUR) > 1)
	{
		board.get_fast_actions(false, FOUR, actions, actions[0]);
		return board.player != player ? 1 : 0;
	}
	...
	if (board.player == player)
	{
		...
		{
			tmp_actions[0] = 0;
			board.get_fast_actions(false, OPEN_THREE, tmp_actions, tmp_actions[0]);
			for (int idx = 1; idx <= tmp_actions[0]; idx++)
			{
				if (board.check_action(true, OPEN_TWO, tmp_actions[idx]))
				{
					actions[++actions[0]] = tmp_actions[idx];
				}
			}
		}
		else
		{
			board.get_fast_actions(true, OPEN_TWO, actions, actions[0]);
		}

		int tmp_count = actions[0];
		bool check_repeat = tmp_count > 0;

		board.get_fast_actions(true, THREE, actions, actions[0]);

		check_repeat = check_repeat & (actions[0] > tmp_count);
		...
		if (check_repeat)
		{
			...
			tmp_count = actions[0];
			actions[0] = 0;
			for (int idx = 1; idx <= tmp_count; idx++)
			{
				if (hash[actions[idx]] == 1)
				{
					continue;
				}
				hash.set(actions[idx] + 1);
				actions[++actions[0]] = actions[idx];
			}
		}
	...
	else
	{
		...
		{
			board.get_fast_actions(true, THREE, actions, actions[0]);
			board.get_fast_actions(false, OPEN_THREE, actions, actions[0]);
...
int fastVct(FastBoard &board, int max_depth, double max_time)
{
	...
	value = fast_evaluate(copy_board, 0, max_depth, shared_actions + shared_index, cache_hashing_table, player);
	if (value >= 0)
	{
		if (value)
		{
	...
	FAST_NODE_TABLE[FAST_NODE_NUMBER] = FastNode(FAST_NODE_NUMBER, OR, 0, cache_hashing_table);
		   ...
		   (double)(clock() - start) / CLOCKS_PER_SEC < max_time)
	{
		...
		for (idx = 1; idx <= shared_actions[tmp_action_index]; idx++)
		{
			...
			child_board.move(tmp_action, true);
			...
			shared_values[++shared_values[tmp_action_index] + tmp_action_index] = fast_evaluate(child_board,
																				  depth, max_depth, shared_actions + shared_index,
		...
		node.develop(shared_actions + tmp_action_index + 1, shared_values + tmp_action_index + 1,
					 shared_actions[tmp_action_index], cache_hashing_table);
